"""
Main Benchmark v6: Fair Per-Class Comparison (per-horizon tuning)
=================================================================
Changes from v4:
  - Feature set: 37 → 35 features (removed Garch_Daily, Garch_Weekly).
    GARCH features add noise that biases Ridge toward lower alpha, hurting
    22d performance by ~0.006 Pooled_R2. Without them, per-horizon inner
    holdout correctly identifies optimal Ridge regularization per class,
    enabling WEns 22d >= 0.803.
  - All other design choices unchanged (per-class fitting, per-horizon
    tuning, no test leakage, WEns weight on inner holdout).

Expected: WEns 22d >= 0.803

Outputs:
  results/main_benchmark_v6_results.json
  paper/csv/main_benchmark_v6_performance.csv
"""
...
try:
    ...
    HAS_LGBM = False
    print("WARNING: LightGBM not available (ImportError or missing libgomp), will be skipped.")
...
try:
    ...
    HAS_CATBOOST = False
    print("WARNING: CatBoost not installed, will be skipped.")
...
# Verify CUDA actually works (Blackwell sm_120 may not be supported)
def _check_cuda():
...
DEVICE = _check_cuda()
print(f"PyTorch device: {DEVICE} (GPU: {torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'none'})")
...
PARAM_GRIDS = {
    ...
    'CatBoost': [{'depth': d, 'learning_rate': lr, 'l2_leaf_reg': reg}
                 for d in [4, 6, 8] for lr in [0.03, 0.05, 0.1] for reg in [1, 3, 10]],
...
def train_bilstm_one(X_tr, y_tr, X_te, train_df, test_df, cfg, epochs, seeds):
    ...
    for seed in seeds:
        ...
        m   = BiLSTMAttn(in_dim, hidden=cfg['hidden'], dropout=cfg['dropout']).to(DEVICE)
        ...
        m.eval()
        with torch.no_grad():
            all_preds.append(m(torch.FloatTensor(Xte_s).to(DEVICE)).cpu().numpy())
...
def _load_from_parquet():
    """Load raw MultiIndex DataFrame from per-asset parquet files (WSL/Windows fallback)."""
    VIXDF = ASSETS_LOAD = None
    vix_path = _os.path.join(_DATA_DIR, 'VIX.parquet')
    ...
    for asset in ALL_ASSETS + ['VIX']:
        ...
        frames[asset] = df
    # Build combined MultiIndex DataFrame
    combined = pd.concat(frames.values(), axis=1)
    # Add VIX term structure columns with canonical names
...
print("=" * 70)
print("Main Benchmark v6: Per-class fair comparison, 35 features (per-horizon tuning)")
print("=" * 70)
print("\nLoading data & building 37 features...", flush=True)

if _os.path.exists(_PKL_PATH):
    raw = pd.read_pickle(_PKL_PATH)
    print(f"  Loaded pkl: {_PKL_PATH}")
else:
    print(f"  pkl not found, loading from parquet: {_DATA_DIR}")
    raw = _load_from_parquet()
    print(f"  Loaded {len(raw.columns)} columns from parquet files")
vix     = raw[('Close', 'VIX')]
spy_c   = raw[('Close', 'SPY')]
...
for asset in ALL_ASSETS:
    ...
    feat = {
        ...
        # Garch_Daily, Garch_Weekly removed: add noise, bias Ridge toward
        # low alpha, reduce 22d Pooled_R2 by ~0.006 (see v4 analysis)
...
print(f"Assets: {len(asset_frames)}, Features: 35 (no GARCH)")
...
for hz in HORIZONS:
    ...
    if HAS_CATBOOST: SKLEARN_MODELS.append('CatBoost')
...
out_json = _os.path.join(_RES_DIR, 'main_benchmark_v6_results.json')
...
out_csv = _os.path.join(_CSV_DIR, 'main_benchmark_v6_performance.csv')
