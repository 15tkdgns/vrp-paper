...
# 252 trading days ≈ 1 calendar year. Annualization factor = 252.
HORIZONS = [1, 5, 22, 63, 126, 252]
...
def forward_rv(ret_sq, horizon):
    ...
    return np.log(fwd_mean * 252 + 1e-12)  # annualized with 252 trading days
...
for horizon in HORIZONS:
    ...
    # Purge: remove h samples at train/test boundary to prevent target leakage
    # Train target uses returns up to t+h, so last h train samples' targets
    # overlap with early test period returns.
    purge_gap = horizon
    split = int(len(data) * 0.8)
    train_df = data.iloc[:split - purge_gap]
