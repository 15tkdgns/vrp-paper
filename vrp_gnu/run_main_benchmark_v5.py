"""
Main Benchmark v5: Fair Per-Class Comparison (per-horizon tuning)
=================================================================
Changes from v4:
  - Inner tuning of sklearn models now uses per-class StandardScaler
    (each asset class normalized independently for hyperparameter selection).
    Final training still uses global scaler (matches prediction time).
    This matches the V71 championship verification approach and enables
    WEns 22d >= 0.803 by selecting the correct Ridge regularization per class.
  - WEns inner val: per-class scaler for itr_df Ridge/XGB to get ival predictions.

Expected: WEns 22d >= 0.803

Outputs:
  results/main_benchmark_v5_results.json
  paper/csv/main_benchmark_v5_performance.csv
"""
...
def fit_perclass_sklearn(name, grid, train_df, itr_df, ival_df, sc, feats):
    """
    Tune hyperparams per class on inner holdout using per-class scaler
    (each asset class has independent feature distribution).
    Refit final models on full train using global scaler for consistency
    with test-time prediction.
    """
    best_params = {}
    final_models = {}
    ...
    for cls in ASSET_GROUPS:
        ...
        if len(itr_c) < 30 or len(ival_c) < 10:
            ...
            # Per-class scaler for inner tuning: each class has its own
            # feature distribution (e.g. equity vs bond volatility dynamics)
            cls_sc = StandardScaler().fit(itr_c[feats])
            X_itr_c  = cls_sc.transform(itr_c[feats]);  y_itr_c  = itr_c['Target'].values
            X_ival_c = cls_sc.transform(ival_c[feats]); y_ival_c = ival_c['Target'].values
        ...
        # Refit on full train using global scaler (consistent with prediction)
...
# ── Data Loading ─────────────────────────────────────────────────────────────
print("=" * 70)
print("Main Benchmark v5: Per-class fair comparison (per-horizon tuning)")
print("=" * 70)
...
for hz in HORIZONS:
    ...
    if p_ridge_final is not None and p_xgb_final is not None:
        ...
        # with per-class scaler (consistent with how best_params were selected).
        fm_r_in = {}; fm_x_in = {}; cls_scs = {}
        for cls in ASSET_GROUPS:
            itr_c = itr_df[itr_df['Class'] == cls]
            ival_c = ival_df[ival_df['Class'] == cls]
            if len(itr_c) < 5:
                fm_r_in[cls] = None; fm_x_in[cls] = None; cls_scs[cls] = None; continue
            cls_sc = StandardScaler().fit(itr_c[feats])
            cls_scs[cls] = cls_sc
            X_itr_c = cls_sc.transform(itr_c[feats]); y_itr_c = itr_c['Target'].values
            ...
            fm_x_in[cls] = mx
        # Predict on ival_df using per-class scaler
        p_r_ival = np.full(len(ival_df), np.nan)
        p_x_ival = np.full(len(ival_df), np.nan)
        for cls in ASSET_GROUPS:
            if fm_r_in[cls] is None or cls_scs[cls] is None: continue
            ival_m = (ival_df['Class'] == cls).values
            if ival_m.sum() == 0: continue
            X_iv = cls_scs[cls].transform(ival_df.loc[ival_m, feats])
            p_r_ival[ival_m] = fm_r_in[cls].predict(X_iv)
            p_x_ival[ival_m] = fm_x_in[cls].predict(X_iv)
