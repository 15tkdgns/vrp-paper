"""
Main Benchmark v3: Fair Per-Class Comparison (per-horizon tuning)
=================================================================
Changes from v2:
  - Global pooled fitting → Per-class fitting (Equity / Bond / Commodity)
    * Same treatment for ALL models → fair comparison
    * Asset classes have structurally different volatility dynamics
  - Same tuning budget: 6 configs × 3 classes inner fits per model
  - BiLSTM-A: 4 configs × 3 classes (time constraint)
  - WEns: reuses already-tuned Ridge + XGBoost (no extra budget),
           weight tuned on inner holdout (no test leakage)

Expected: WEns 22d >= 0.803

Outputs:
  results/main_benchmark_v3_results.json
  paper/csv/main_benchmark_v3_performance.csv
"""
...
# ── Data Loading ─────────────────────────────────────────────────────────────
print("=" * 70)
print("Main Benchmark v3: Per-class fair comparison (per-horizon tuning)")
print("=" * 70)
...
for hz in HORIZONS:
    ...
    if p_ridge_final is not None and p_xgb_final is not None:
        ...
        # Re-predict on inner val using per-class models refitted on itr
        _, fm_r_in = fit_perclass_sklearn(
            'Ridge', PARAM_GRIDS['Ridge'],
            itr_df, itr_df.iloc[:int(len(itr_df)*0.8) - hz],
            itr_df.iloc[int(len(itr_df)*0.8):], sc, feats)
        _, fm_x_in = fit_perclass_sklearn(
...
out_json = '/root/vrp/results/main_benchmark_v3_results.json'
...
out_csv = '/root/vrp/paper/csv/main_benchmark_v3_performance.csv'
