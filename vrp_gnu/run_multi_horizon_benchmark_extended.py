...
def train_bilstm(X_tr, y_tr, X_te, seq_len=22, hidden=32, lr=0.001, epochs=20, seeds=[0,1,2,3,4]):
    """Train BiLSTM-A with multiple seeds and average predictions."""
    ...
    for seed in seeds:
        ...
        Xte_s, yte_s = make_seq(X_te, np.zeros(len(X_te)))  # y_te not needed for pred

        model = BiLSTMAttention(input_dim=X_tr.shape[1], hidden_dim=hidden).to(device)
    ...
    return avg_preds  # length = len(X_te) - seq_len
...
for hz in HORIZONS:
    ...
    m_rf = RandomForestRegressor(n_estimators=200, max_depth=10, min_samples_leaf=20,
    ...
    print(f"R2={r2_wens:.4f} (XGB: {r2_xgb:.4f})")
    ...
    bl_preds = train_bilstm(X_tr_bl, y_tr_bl, X_te_bl, seq_len=seq_len)
    # BiLSTM predictions are shorter by seq_len
    y_te_bl = y_te[seq_len:]
    r2_bilstm = r2_score(y_te_bl, bl_preds)
    ...
    hz_key = f"{hz}d"
    results[hz_key] = {
        'HAR-3':     {'Pooled_R2': round(r2_har, 4),    'RMSE': round(np.sqrt(mean_squared_error(y_te, p_har)), 4)},
        'Ridge':     {'Pooled_R2': round(r2_ridge, 4),   'RMSE': round(np.sqrt(mean_squared_error(y_te, p_ridge)), 4)},
        'LASSO':     {'Pooled_R2': round(r2_lasso, 4),   'RMSE': round(np.sqrt(mean_squared_error(y_te, p_lasso)), 4)},
        'ElasticNet':{'Pooled_R2': round(r2_enet, 4),    'RMSE': round(np.sqrt(mean_squared_error(y_te, p_enet)), 4)},
        'SVR':       {'Pooled_R2': round(r2_svr, 4),     'RMSE': round(np.sqrt(mean_squared_error(y_te, p_svr)), 4)},
        'MLP':       {'Pooled_R2': round(r2_mlp, 4),     'RMSE': round(np.sqrt(mean_squared_error(y_te, p_mlp)), 4)},
        'RF':        {'Pooled_R2': round(r2_rf, 4),      'RMSE': round(np.sqrt(mean_squared_error(y_te, p_rf)), 4)},
        'XGBoost':   {'Pooled_R2': round(r2_xgb, 4),     'RMSE': round(np.sqrt(mean_squared_error(y_te, p_xgb)), 4)},
        'LightGBM':  {'Pooled_R2': round(r2_lgbm, 4),    'RMSE': round(np.sqrt(mean_squared_error(y_te, p_lgbm)), 4)},
        'WEns':      {'Pooled_R2': round(r2_wens, 4),    'RMSE': round(np.sqrt(mean_squared_error(y_te, p_wens)), 4)},
        'BiLSTM-A':  {'Pooled_R2': round(r2_bilstm, 4),  'RMSE': round(np.sqrt(mean_squared_error(y_te_bl, bl_preds)), 4)},
    }
...
# ── Save ────────────────────────────────────────────────────
out_path = '/root/vrp/multi_horizon_benchmark_extended_results.json'
