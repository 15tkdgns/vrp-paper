...
def run():
    ...
    print("\n--- Diebold-Mariano Test (Newey-West HAC, horizon=22) ---", flush=True)
    print(f"{'Comparison':<40} {'DM-stat':>10} {'p-value':>10} {'Result':>10}", flush=True)
    print("-"*75, flush=True)
    ...
    for model_a, model_b in comparisons:
        ...
        dm_stat, p_val = diebold_mariano_test(e_a, e_b, horizon=22)
        sig = "***" if p_val < 0.01 else "**" if p_val < 0.05 else "*" if p_val < 0.1 else "n.s."
        label = f"{model_a} vs {model_b}"
        dm_results[label] = {'dm_stat': dm_stat, 'p_value': p_val, 'significance': sig}
        print(f"{label:<40} {dm_stat:>10.4f} {p_val:>10.6f} {sig:>10}", flush=True)
