...
| **22d** | **0.803** | 0.215 | 0.468 |
| 60d | 0.774 | -0.201 | 0.471 |

- **단기(1~22d) 풀링 R² 최고**: 37개 피처의 풍부한 정보가 단기 예측에 유리
- 문헌 벤치마크 HAR-RV(Corsi 2009, 22d: 0.761), HAR-CJ(Andersen+ 2007, 22d: 0.752), LASSO(22d: 0.790), RF(22d: 0.770) 대비 우위
- 6개 건전성 검증 테스트 **전체 통과**
...
> **주의**: 위 R² 수치들은 V71 원래 실험(중첩된 RV 타겟) 기준. 전방 RV 기준으로는 22d R²=0.803 (Ensemble), 0.798 (Ridge 단독), 0.780 (LSTM Top3), 0.761 (HAR-3).

### 2.2 핵심 발견

1. **예측 기간별 최적 모델 분화**: 단기(1~22d) Ridge > 중기(60~90d) LSTM > 초장기(365d) RF
...
[자산별 OHLCV 데이터]
    ...
    → 22d 풀링 R² = 0.798 (Ridge only), 0.803 (Ensemble)
```
...
**결론**: R² = 0.798 +/- 0.010 (Ridge 단독), 모든 split에서 0.775 이상으로 안정적
...
| **V36** | HAR-3 Ridge | 3 | 0.761 | 기준 |
| **V71** | Ensemble | 37 | **0.803** | **+5.5%** |
| V50 Tuned | Bi-LSTM | 3 | 0.780 | +2.5% |
| V43 | Transformer | 2 | 0.534 (overlapping) | - |
| **V71 Ensemble** | **Ridge+XGBoost** | **37** | **0.803 (overlapping)** | - |

> 위 테이블의 V36/V71/V50 Tuned는 전방 RV 기준, V43/V71 Ensemble은 overlapping 기준.
