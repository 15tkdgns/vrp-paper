...
V6_JSON = '/root/vrp/results/main_benchmark_v6_results.json'
...
if HAS_LGBM:
    ...
    MODELS_V6_EXTRA = ['MLP']
MODELS_NEW = ['NaiveRV', 'IV-only', 'GARCH']
...
for hz in HORIZONS:
    ...
    for asset in ALL_ASSETS:
        ...
        # Naive RV: h-day trailing mean ret_sq (past), log-annualised
    ...
    feats = [c for c in data.columns
             if c not in ['Target', 'Asset', 'Class', 'NaiveRV_pred']]
...
# JSON
out_json = '/root/vrp/results/baseline_comparison_results.json'
...
out_perf = '/root/vrp/paper/csv/baseline_comparison_performance.csv'
df_perf.to_csv(out_perf, index=False)
print(f"Saved: {out_perf}")

# DM Test CSV
df_dm = pd.DataFrame(dm_results)
out_dm = '/root/vrp/paper/csv/dm_test_results.csv'
